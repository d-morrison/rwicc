...
Estimation" by Morrison, Laeyendecker, and Brookmeyer (2021) in Biometrics: \doi{10.1111/biom.13472}.
...
```{r}
knitr::kable(head(sim_participant_data), caption = "Simulated participant data")
```

Here's a look at the observation-level (longitudinal) data:

```{r}
knitr::kable(head(sim_obs_data), caption = "Simulated observation data")
```

We can see that the two tables are linked by the variable ID.
...
Here we implement an alternative analysis using midpoint imputation:
